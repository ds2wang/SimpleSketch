...
public class MyRectangle extends MyBoundedShape
{
    ...
    public MyRectangle( int x1, int y1, int x2, int y2, int thickness, Color color, Color color2, boolean gradient, boolean filled, boolean dashed, int dlength )
    {
    ...
    public MyRectangle( int x1, int y1, int x2, int y2, int thickness, Color color, Color color2, boolean gradient, boolean filled, boolean dashed, int dlength, int isEraser )
    {
        super( x1, y1, x2, y2, thickness, color, color2, gradient, filled, dashed, dlength );//calls super constructor
    ...
    public void draw( Graphics g )
    {
        ...
        if ( getFilled() ){//checks whether or not to fill shape
            g2.fillRect( getUpperLeftX(), getUpperLeftY(), getWidth(), getHeight() );;//draws filled rectangle
        } else{
        	if(isEraser==0){
        		g2.drawRect( getUpperLeftX(), getUpperLeftY(), getWidth(), getHeight() );//draws a rectangle with the specified color
        	}else{
        		g2.drawRect( getUpperLeftX(), getUpperLeftY(), getWidth(), getHeight() );//draws a rectangle with the specified color
        		g2.setColor(Color.WHITE);
        		g2.fillRect( getUpperLeftX()+1, getUpperLeftY()+1, getWidth()-1, getHeight()-1 );;//draws filled rectangle
