...
public class MyRoundRect extends MyBoundedShape
{
    ...
    public MyRoundRect( int x1, int y1, int x2, int y2, int thickness, Color color, Color color2, boolean gradient, boolean filled, boolean dashed, int dlength )
    {
    ...
    public void draw( Graphics g )
    {
        ...
        if (getDashed()) //check if to make the lines dashed
            g2.setStroke (new BasicStroke(getThickness(), BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND,  3f, new float[] {getDLength()}, lol));
        else
            g2.setStroke(new BasicStroke(getThickness()));
        if ( getFilled() ){ //checks whether or not to fill shape
            g2.fillRoundRect( getUpperLeftX(), getUpperLeftY(), getWidth(), getHeight(), 20, 20 );;//draws filled rounded rectangle
        } else{
            g2.drawRoundRect( getUpperLeftX(), getUpperLeftY(), getWidth(), getHeight(), 20 ,20 );//draws a  rounded rectangle with the specified color
        }
    } // end method draw
